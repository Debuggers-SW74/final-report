...
[![tellosprint1.png](https://i.postimg.cc/XqYr3Wds/TrelloSprint1.png)](https://postimg.cc/XGtjcT25)
...
[![sprint1evidence.png](https://i.postimg.cc/851mjw3s/Evidence-Sprint1.png)](https://postimg.cc/cKbnz7dN)
...
Para esta entrega todos los miembros del equipo colaboraron en el desarrollo de las primeras veresiones del Landing Page y Web Application:
...
| Repository                        | Branch         | Commit Id                                | Commit Message                                     | Commit Message Body | Commited on (Date) |
| --------------------------------- | -------------- | ---------------------------------------- | -------------------------------------------------- | ------------------- | ------------------ |
| Debuggers-SW74/mobile-application | main           | 7fb57a5ac5323c26dda7e7dff7368ff863528b6d | Initial commit                                     |                     | 14/10/2024         |
...
[![busqueda.png](https://i.postimg.cc/x8W41qw5/search-Driver.png)](https://postimg.cc/BPBghZQP)

Se agrego la sección de edición de Perfil para la actualización de los datos:
[![profile.png](https://i.postimg.cc/wjYrKycT/notifications-web.png)](https://postimg.cc/zVp7Kfc9)

Se crearon las notificaciones base que se mostrarán en la aplicación:
[![notifications.png](https://i.postimg.cc/wjYrKycT/notifications-web.png)](https://postimg.cc/zVp7Kfc9)

Se agrego la sección de Soporte para redirigir a la Landing Page y brindar soporte:

[![support.png](https://i.postimg.cc/59GG3kXz/support-Web.png)](https://postimg.cc/pyfqVCYV)
...
[![cloudApi1.png](https://i.postimg.cc/WpnmpDv1/cloud-Api1.png)](https://postimg.cc/DmS4Bw09)
[![cloudApi2.png](https://i.postimg.cc/28SQ1zkF/cloud-Api2.png)](https://postimg.cc/bd4DMf2s)
...
| ---------------------------------------------- | -------------- | ---------------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Iniciar un viaje                               | PUT            | /api/v1/trips/{id}/starts/                                 | `id`: ID del viaje                                           | `200 OK`                                                         | Inicia el viaje con el ID proporcionado.                                       |
| Finalizar un viaje                             | PUT            | /api/v1/trips/{id}/completions                             | `id`: ID del viaje                                           | `200 OK`                                                         | Finaliza el viaje con el ID proporcionado.                                     |
| Cancelar un viaje                              | PUT            | /api/v1/trips/{id}/cancellations                           | `id`: ID del viaje                                           | `200 OK`                                                         | Cancela el viaje con el ID especificado.                                       |
...
| Consultar un viaje específico                  | GET            | /api/v1/trips/{id}                                         | `id`: ID del viaje                                           | `{ "tripId": 123, "status": "PENDING" }`                         | Devuelve el estado actual del viaje con el ID especificado.                    |
| Consultar viajes por supervisor                | GET            | /api/v1/trips/supervisor/{supervisorId}                    | `supervisorId`: ID del supervisor                            | `[ { "tripId": 123, "status": "PENDING" }, ... ]`                | Devuelve los viajes asignados al supervisor especificado.                      |
| Consultar viajes por estado para un supervisor | GET            | /api/trips/supervisor/<br>{supervisorId}/status/{statusId} | `supervisorId`: ID del supervisor, `statusId`: ID del estado | `[ { "tripId": 123, "status": "FINISHED" }, ... ]`               | Devuelve los viajes con un estado específico para el supervisor.               |
| Consultar viajes por conductor                 | GET            | /api/v1/trips/driver/{driverId}                            | `driverId`: ID del conductor                                 | `[ { "tripId": 123, "status": "PENDING" }, ... ]`                | Devuelve los viajes asignados al conductor especificado.                       |
| Consultar viajes por estado para un conductor  | GET            | /api/v1/trips/driver/<br>{driverId}/status/{statusId}      | `driverId`: ID del conductor, `statusId`: ID del estado      | `[ { "tripId": 123, "status": "FINISHED" }, ... ]`               | Devuelve los viajes con un estado específico para el conductor.                |
| Registrar un supervisor                        | POST           | /api/v1/supervisors                                        | JSON con datos del supervisor                                | `{ "supervisorId": 1, "username": "ocanellas" ...}`              | Registra un nuevo supervisor y devuelve su ID con información de su cuenta.    |
| Actualizar información de un supervisor        | PUT            | /api/v1/supervisors                                        | JSON con datos actualizados del supervisor                   | `{ "supervisorId": 1, "status": "ocanellas" ...}`                | Actualiza la información de un supervisor existente.                           |
| Consultar todos los supervisores               | GET            | /api/v1/supervisors                                        | Ninguno                                                      | `[ { "supervisorId": 1, "name": "Supervisor 1" ...}, ... ]`      | Devuelve una lista de todos los supervisores.                                  |
| Consultar un supervisor específico             | GET            | /api/v1/supervisors/{id}                                   | `id`: ID del supervisor                                      | `{ "supervisorId": 1, "name": "Supervisor 1" ...}`               | Devuelve la información del supervisor con el ID especificado.                 |
...
| Registrar múltiples conductores                | POST           | /api/v1/drivers/batch                                      | JSON con lista de conductores                                | `[ { "driverId": 101, "username": "pgomez" ... }, ... ]`         | Registra múltiples conductores en una sola llamada.                            |
...
| Consultar conductores por supervisor           | GET            | /api/v1/drivers/<br>supervisor/{supervisorId}              | `supervisorId`: ID del supervisor                            | `[ { "driverId": 101, "name": "Driver 1" }, ... ]`               | Devuelve una lista de conductores supervisados por el supervisor especificado. |
...
| Configurar umbrales de seguridad               | POST           | /api/v1/thresholds                                         | JSON con detalles de los umbrales                            | `{ "thresholdId": 1, "maxThreshold": 95.1 }`                     | Configura un conjunto de umbrales de seguridad para el sistema.                |
| Actualizar umbrales de seguridad               | POST           | /api/v1/thresholds                                         | JSON con datos actualizados de umbrales                      | `{ "thresholdId": 1, "maxThreshold": 80 }`                       | Actualiza los umbrales de seguridad configurados.                              |
| Consultar umbrales de un viaje                 | GET            | /api/v1/thresholds/trip/{tripId}                           | `tripId`: ID del viaje                                       | `[ { "sensorType": "SENSOR_GAS", "sensorType": "5", ... }, ...]` | Devuelve los umbrales configurados para un viaje específico.                   |
...
| Consultar alertas por viaje                    | GET            | /api/v1/alerts/trip/{tripId}                               | `tripId`: ID del viaje                                       | `[ { "alertId": 789, "alertLevel": "CRITICAL" }, ... ]`          | Devuelve una lista de alertas generadas para un viaje específico.              |
| Consultar estados de viajes                    | GET            | /api/v1/trips-status                                       | Ninguno                                                      | `[ { "id": 1, "status": "CANCELED" }, ... ]`                     | Devuelve una lista de todos los estados de viaje disponibles.                  |
...
- **Supervisor-Management**: Proporciona endpoints para la administración de supervisores, incluyendo registro, actualización y eliminación de supervisores.
     [![supervisors.png](https://i.postimg.cc/TwySMZ1n/trips-api.png)](https://postimg.cc/wypFD4hB)
- **Driver-Management**: Contiene endpoints para la gestión de conductores, como el registro de nuevos conductores y la actualización de información.
     [![driverss.png](https://i.postimg.cc/hGt6MJqy/driver-api.png)](https://postimg.cc/JsSpzz0Z)
 - authentication-controller: Contiene endpoints para la autenticación y autorización de usuarios.
     [![auth.png](https://i.postimg.cc/vZkf2jJJ/auth.png)](https://postimg.cc/v474cPHh)
...
![SoDE1](./assets/chapter6_img/deploy_evidence_webapp_0.jpg)
#### **Despliegue del Cloud API y Edge API**
...
![SoDE1](./assets/chapter6_img/deploy_evidence_cloudapi_1.jpg)
...
![SoDE1](./assets/chapter6_img/deploy_evidence_edgeapi_1.jpg)

Y en GitHub Action lo podemos ver de esta forma
![SoDE1](./assets/chapter6_img/deploy_evidence_edgeapi_1_1.jpg)
...
**Cloud Database:** 
![SoDE1](./assets/chapter6_img/deploy_evidence_clouddb1.jpg)

**Edge Database**

![SoDE1](./assets/chapter6_img/deploy_evidence_edgedb1.jpg)
...
[![landingpage.png](https://i.postimg.cc/9XNW5hfN/insigths-landingpage-sprint2.png)](https://postimg.cc/SjC0CHCW)
...
[![webapp.png](https://i.postimg.cc/hvhczC2r/insigths-web-application-sprint2.png)](https://postimg.cc/vxRpCvFg)
...
[![cloudserver.png](https://i.postimg.cc/wMPHhhPd/insigths-web-services-sprint2.png)](https://postimg.cc/hQ9Hn7cC)
...
[![iotapp.png](https://i.postimg.cc/3Rt3wMFN/insigths-iot-app-sprint2.png)](https://postimg.cc/yg3Mfb5C)
...
[![tellosprint3.png](https://i.postimg.cc/mDGW9k1p/Trello-Sprint3.png)](https://postimg.cc/4Kw0kJ5p)
...
| Repository                  | Branch | Commit Id | Commit Message | Commit Message Body | Commited on (Date) |
| --------------------------- | ------ | --------- | -------------- | ------------------- | ------------------ |
| Debuggers-SW74/landing-page | main   | 80afb9607b3217139b219e0a000414bbbba0fdbe| refactor: update links |             |  Nov 3, 2024|
|                             | main   | e28839935ef8cf4df1384e7e0dd4eec7f6c96250| Merge pull request #13 from Debuggers-SW74/develop |             |  Nov 3, 2024|
|                             | main   | 28f742b62006d782e302b49c5e27c43548dd4710| feat: video about the team |             |  Nov 3, 2024|
|                             | main   | ef755600f2024bbff92dcf85ac9ac450d9bff619| Merge pull request #12 from Debuggers-SW74/develop |             |  Nov 2, 2024|
|                             | main   | 27ac8687c8519ee6ca3e55ab18793cca8d94aee0| feat: video about the product |             |  Nov 2, 2024|
...
| Repository                        | Branch | Commit Id | Commit Message | Commit Message Body | Commited on (Date) |
| --------------------------------- | ------------------------- | ---------------------------------------- | ------------------------------------------------ | ------------------- | ------------------ |
| Debuggers-SW74/mobile-application | feature/supervisor/charts   | f192d4b692aeeacd7fdc84d63fc0a66ea8ab68b1 | fix: sensor data type fixed to double |   | Nov 20, 2024|
|                                   | feature/supervisor/charts   | b496db0c29ce5f00d76cddafd1a00623bdbb08ca | fix: alerts and chart values |   | Nov 20, 2024|
|                                   | feature/supervisor/charts   | e0ca8526d7233ded7a9af33580f3dbadce68fd63 | fix: just alerts from 40 seconds |   | Nov 20, 2024|
|                                   | feature/supervisor/charts   | a7c2cea403e021782f74a0a8342546f887fa49d7 | feat: alerts |   | Nov 20, 2024|
|                                   | feature/supervisor/charts   | b03eeb352d1215ac479dc9b8ccae343b9a4a44cf| feat: current data trip with future build |   | Nov 19, 2024|
|                                   | feature/supervisor/charts   | 2a44210578e708a343346b336651c8f200dc6f3f| feat: temperature chart base |   | Nov 19, 2024|
|                                   | feature/supervisor/charts   | 7ed4e7f5c83aa955d2af2bd1a54d3d27642806c5| feat: buttons to change trip status added |   | Nov 19, 2024|
|                                   | feature/supervisor/charts   | 24f6976e2e849c9d2dbabb251bdf3bca080ff1b5| refactor: rename files |   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | b5cc8a6a5a5a4f8fd0856581adc08aa88fc70195| refactor: clean unused views |   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | 64a7462a3cd50299ece29036ddd398c88e5ce8e9| refactor: versions updated to avoid ndk warnings |   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | 042e19b38041b48c411cf2442d34a19eedce88d1| Merge branch 'feature/driver/charts' into test |   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | 83715ebe7ff4f589d32fa702262840c6de497bd4| fix: register users validtions |   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | 72d23a58350602d87d8c50bac6dfb9239fc84ad9| feat: support view added|   | Nov 18, 2024|
|                                   | feature/supervisor/charts   | 1e5d7a67aa15e89c92851471705ccab83b9f5316| feat(threshold): check values|   | Nov 18, 2024|

Tablero de commits del repositorio del Cloud Web Server:

| Repository                  | Branch  | Commit Id                                | Commit Message                                 | Commit Message Body | Commited on (Date) |
| --------------------------- | ------- | ---------------------------------------- | ---------------------------------------------- | ------------------- | ------------------ |
| Debuggers-SW74/web-services | develop | 86cfde00657f3ebf7119bcc3b77c32dd0391b113 | fix: alert resource fixed to receive timestamp |                     | Nov 20, 2024       |
|                             | develop | 85f49c49396f0f3dcaaa239e2d4665bb81e8e35b | fix: zone time fixed to Peru                   |                     | Nov 19, 2024       |
|                             | develop | d111909b7a63aac8dd5a65d62d16f2e5cf2e665d | feat: add real time sensor data endpoints      |                     | Nov 19, 2024       |
|                             | develop | f1d8e57ad0036e3fafc2d910c59d366ef1d989e1 | fix: create trip constraints fixed             |                     | Nov 17, 2024       |
|                             | develop | e0938ef3d221e3cd3d636e2559ed359481439f58 | fix: trip fields validation fixed              |                     | Nov 15, 2024       |
|                             | develop | 8bfe170282e9600e1f96c7e40f37bf24415c487f | refactor: add fields to the trip information   |                     | Nov 15, 2024       |
|                             | develop | e0938ef3d221e3cd3d636e2559ed359481439f58 | fix: trip fields validation fixed              |                     | Nov 15, 2024       |
|                             | develop | 083b9d7b309e94d7dad7b7407ae8a16a7642a766 | fix: trip information fixed                    |                     | Nov 15, 2024       |
|                             | develop | e56f401001e86fe5835662a4ec7ed6a8aef29554 | fix: show user phone added                     |                     | Nov 15, 2024       |
...
| Repository              | Branch | Commit Id                                | Commit Message                                 | Commit Message Body | Commited on (Date) |
| ----------------------- | ------ | ---------------------------------------- | ---------------------------------------------- | ------------------- | ------------------ |
| Debuggers-SW74/edge-api | main   | c978d9450f7d0787e4a225b8b265936c2c710f23 | feat: Sending data to the Cloud API successful |                     | Nov 19, 2024       |
| Debuggers-SW74/edge-api | main   | 80213fa71c11cef1f474914a750d1b19647908eb | feat: send data updated                        |                     | Nov 19, 2024       |
| Debuggers-SW74/edge-api | main   | 2f3fb585d90391456e568404460cb40099427129 | feat: real time data updated                   |                     | Nov 19, 2024       |
| Debuggers-SW74/edge-api | main   | da3c3aeae596e6dddefd815152274f94b8ba7e10 | feat: reading was added to the Cloud API       |                     | Nov 18, 2024       |
| Debuggers-SW74/edge-api | main   | c7b029523ef4d80043b247c8e571573167e13cc2 | feat: the gas sensor was added                 |                     | Nov 18, 2024       |
| Debuggers-SW74/edge-api | main   | 3ae716e00878c69564d6ae8b0b755e165329ac6c | feat: update state endpoint                    |                     | Nov 17, 2024       |
| Debuggers-SW74/edge-api | main   | f0d02e97f44eb5e20cc3ebddb10b82940906147a | feat: updated alert endpoint                   |                     | Nov 17, 2024       |
...
![SoDE1](./assets/chapter6_img/deploy_evidence_webapp_1.jpg)
![SoDE1](./assets/chapter6_img/deploy_evidence_webapp_1_1.jpg)
![SoDE1](./assets/chapter6_img/deploy_evidence_webapp_1_2.jpg)
...
![SoDE1](./assets/chapter6_img/deploy_evidence_mobile1.jpg)
![SoDE1](./assets/chapter6_img/deploy_evidence_mobile2.jpg)
...
![SoDE1](./assets/chapter6_img/deploy_evidence_cloudapi_21.jpg)
   
   En esta imagen se puede ver la consola de GitHub Actions también con los commits especificados en la imagen previa.
   
![SoDE1](./assets/chapter6_img/deploy_evidence_cloudapi_22.jpg)

**Edge API:**

De igual forma, aquí se ve la evidencia de despliegue en la consola de Azure con la fecha y hora correspondiente junto con el commit relacionado.
![SoDE1](./assets/chapter6_img/deploy_evidence_edgedapi_21.jpg)

Y en GitHub Action lo podemos ver de esta forma
![SoDE1](./assets/chapter6_img/deploy_evidence_edgedapi_22.jpg)

#### Despliegue de Embedded App

La aplicación embebida se encuentra en Wokwi como lo vemos en la siguiente imagen: 

![SoDE1](./assets/chapter6_img/deploy_wokwi.jpg)
...
![SoDE1](./assets/chapter6_img/sprint3_insights_mobile.png)
...
![SoDE1](./assets/chapter6_img/sprint3_insights_edge.png)
...
---
![Supen1](./assets/chapter6_img/entrevistas/entrevista_supervisor_1.png)
<table>
  <tr>
...
---
![Supen1](./assets/chapter6_img/entrevistas/entrevista_supervisor_2.png)
<table>
  <tr>
    <th>Nombres y Apellidos: </th>
    <th>Jorge Chero</th>
...
---
![Supen1](./assets/chapter6_img/entrevistas/entrevista_supervisor_3.png)
<table>
  <tr>
    <th>Nombres y Apellidos: </th>
...
![Supen1](./assets/chapter6_img/entrevistas/entrevista_chofer_1.png)
<table>
  <tr>
    <th>Nombres y Apellidos: </th>
...
![Video About-the-Product](./assets/chapter6_img/about-the-product_s3.png)
